...
if (isset($_POST['simpan'])) {
    ...
    if (empty($error)) {
        if($foto_name){
            $direktori = "../Paskibraka/gambar";

            @unlink($direktori."/$foto"); //delete data

            $foto_name = "events_".time()."_".$foto_name;
        ...
        if($id != ""){
            $sql1   = "update events set nama = '$nama',foto='$foto_name',isi='$isi',tgl_isi=now() where id = '$id'";
        }else{
            $sql1       = "insert into events(nama,foto,isi) values ('$nama','$foto_name','$isi')";
        }
...
?>
<h1>Halaman Admin Input Data Events</h1>
<div class="mb-3 row">
    <a href="events.php">
        << Kembali ke halaman admin Events</a>
</div>
<?php
if ($error) {
?>
...
<form action="" method="post" enctype="multipart/form-data">
    ...
    <div class="mb-3 row">
        ...
        <div class="col-sm-10">
            <?php 
            if($foto){
                echo "<img src='../Paskibraka/gambar/$foto' style='max-height:100px;max-width:100px'/>";
            }
            ?>

...
function events_foto($id){
    ...
    if($foto){
        ...
        return 'events_default_picture.png';
    }
}

function buat_link_events($id){
    global $koneksi;
    $sql1    = "select * from events where id = '$id'";
    $q1     = mysqli_query($koneksi,$sql1);
    $r1     = mysqli_fetch_array($q1);
    ...
    // http://localhost/website-company-profile/halaman.php/8/judul
    return url_dasar()."/events.php/$id/$nama";
}
...
function kirim_email($email_penerima, $nama_penerima,$judul_email,$isi_email){
    ...
    try {
        ...
        $mail->Username   = $user_pengguna;                     //SMTP username
